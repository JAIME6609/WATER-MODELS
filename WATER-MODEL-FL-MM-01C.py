...
# ===============================================================
# =========== INSTRUMENTATION ADD-ON FOR SUPPLEMENTARY ==========
# =========== (Append at the end of WATER-MODEL-FL-MM-01.py) ====
# ===============================================================
...
# ---------------------------------------------------------------
# Safe getters to avoid hard failures if some globals are absent
# ---------------------------------------------------------------

def _g(name, default=None):
    """Safely fetch a global variable; return default if missing."""
    return globals().get(name, default)

def _ensure_dict(name):
    v = _g(name, {})
    return v if isinstance(v, dict) else {}

def _ensure_float(name, default=0.0):
    v = _g(name, default)
    try:
        return float(v)
    except Exception:
...
def _ensure_int(name, default=0):
    v = _g(name, default)
    try:
        return int(v)
    except Exception:
...
# ---------------------------------------------------------------
# Expected globals from the base script (used if present)
# ---------------------------------------------------------------

# Fuzzy/defuzz set-up
type_function = _g("type_function", "triangular")
defuzz_method = _g("defuzz_method", "centroid")

# Core sets/ranges
avail_ranges = _ensure_dict("avail_ranges")          # {i: {"min":..,"nominal":..,"max":..}}
demand_ranges = _ensure_dict("demand_ranges")        # {j: {"min":..,"nominal":..,"max":..}}
cost_params = _ensure_dict("cost_params")            # {i: (cmin, cnom, cmax)}
treatment_capacity_fuzzy = _g("treatment_capacity_fuzzy", (40000, 50000, 60000))

# Temporal factors and indexing
availability_factors = _ensure_dict("availability_factors")  # {t: factor}
demand_factors = _ensure_dict("demand_factors")              # {t: factor}
indices = _g("indices", [])                                  # [(i,j,t), ...]
n_periods = _ensure_int("n_periods", max(demand_factors.keys()) if demand_factors else 1)

# Optimization environment percentage (e.g., sustainable extraction threshold)
environmental_percentage = _g("environmental_percentage", 0.90)

# Cost dictionary per source after defuzzification (if present)
costs = _ensure_dict("costs")  # {i: crisp_cost_per_m3}

# Constraints (use safe defaults if missing; missing constraints are skipped)
A = _ensure_dict("A")                      # availability hard caps per source
D = _ensure_dict("D")                      # minimum demand per sector
Ct = _ensure_float("Ct", 0.0)              # total treatment capacity (hard cap)
B = _ensure_float("B", float("inf"))       # budget cap
Cij = _ensure_dict("Cij")
COij = _ensure_dict("COij")
CEij = _ensure_dict("CEij")
CMij = _ensure_dict("CMij")
CENij = _ensure_dict("CENij")
Aij = _ensure_dict("Aij")                  # aquifer stress multipliers
Cinfra = _ensure_float("Cinfra", 0.0)
Tr = _ensure_dict("Tr")                    # infrastructure/energy multiplier
R_drought = _ensure_float("R_drought", 0.0)
R_strategic = _ensure_float("R_strategic", float("inf"))
L_norm = _ensure_float("L_norm", float("inf"))
B_hidro = _ensure_float("B_hidro", 0.0)
Ds = _ensure_dict("Ds")
Amax = _ensure_float("Amax", float("inf"))

# 44–50 extended policy/quality constraints (optional)
M_max = _ensure_dict("M_max")
M = _ensure_dict("M")
F = _ensure_dict("F")
F_max = _ensure_float("F_max", float("inf"))
INF = _ensure_dict("INF")
INF_max = _ensure_float("INF_max", float("inf"))
Q_mon_min = _ensure_float("Q_mon_min", 0.0)
Q_mon = _ensure_dict("Q_mon")
Minfij = _g("Minfij", None)          # expected 2D list-like indexed [i-1][j-1]
Minf_min = _ensure_float("Minf_min", 0.0)
Emin = _ensure_float("Emin", 0.0)
E_edu = _g("E_edu", None)            # expected 2D list-like indexed [i-1][j-1]

# Algorithms dictionary from the base script
algorithms = _ensure_dict("algorithms")

# Imported fuzzy helpers from the base script
get_membership_functions = _g("get_membership_functions", None)
defuzzify_variable = _g("defuzzify_variable", None)
defuzzify_cost = _g("defuzzify_cost", None)

# Try importing fuzz (from skfuzzy) if the base already imported it
try:
    fuzz  # type: ignore
except NameError:
    try:
        import skfuzzy as fuzz  # safe import if available
    except Exception:
        fuzz = None

# ---------------------------------------------------------------
# Utilities: mapping, vector→dict, CRISP recomputation
# ---------------------------------------------------------------

def vector_to_dict_ext(sol_vector):
    """Same idea as vector_to_dict (base script); ensures use of global 'indices'."""
    sol_dict = {}
    for idx, key in enumerate(indices):
        sol_dict[key] = float(sol_vector[idx])
    return sol_dict

def build_index_mapping_table():
    """Table S5: Index-to (source, sector, period) mapping."""
    rows = []
    for lin_idx, (i, j, t) in enumerate(indices):
        rows.append({
...
def compute_crisp_inputs():
    """
    Recompute CRISP inputs as used in the Word manuscript:
    - CRISP availability per (i, t)
    - CRISP demand per (j, t)
    - CRISP costs per source i
    - CRISP treatment capacity (centroid)
    """
    if get_membership_functions is None or defuzzify_variable is None or fuzz is None:
        _log("Fuzzy helpers not available; CRISP inputs will be approximated from nominal values.")
    crisp_avail = {}
    for i, params in avail_ranges.items():
        if get_membership_functions and defuzzify_variable and fuzz is not None:
            x_av = np.arange(params["min"], params["max"] + 1, 1)
            mf_av = get_membership_functions(x_av, params["min"], params["max"], type_function)
            base_val = defuzzify_variable(x_av, mf_av, params.get("nominal", (params["min"]+params["max"])/2), defuzz_method)
        else:
            base_val = params.get("nominal", (params["min"] + params["max"]) / 2.0)
        crisp_avail[i] = {t: availability_factors.get(t, 1.0) * base_val for t in range(1, n_periods + 1)}

    crisp_dem = {}
    for j, params in demand_ranges.items():
        if get_membership_functions and defuzzify_variable and fuzz is not None:
            x_dm = np.arange(params["min"], params["max"] + 1, 1)
            mf_dm = get_membership_functions(x_dm, params["min"], params["max"], type_function)
            base_val = defuzzify_variable(x_dm, mf_dm, params.get("nominal", (params["min"]+params["max"])/2), defuzz_method)
        else:
            base_val = params.get("nominal", (params["min"] + params["max"]) / 2.0)
        crisp_dem[j] = {t: demand_factors.get(t, 1.0) * base_val for t in range(1, n_periods + 1)}

    crisp_costs = {}
    for i, tpl in cost_params.items():
        if defuzzify_cost is not None:
            crisp_costs[i] = defuzzify_cost(tpl, type_function, defuzz_method)
        else:
            # fallback to nominal
            cmin, cnom, cmax = tpl
            crisp_costs[i] = float(cnom)

    # CRISP treatment capacity (centroid of triangular membership)
    try:
        x_cap = np.linspace(treatment_capacity_fuzzy[0], treatment_capacity_fuzzy[2], 1000)
        if fuzz is not None:
            mu = fuzz.trimf(x_cap, [treatment_capacity_fuzzy[0],
                                    treatment_capacity_fuzzy[1],
                                    treatment_capacity_fuzzy[2]])
            crisp_tr = fuzz.defuzz(x_cap, mu, defuzz_method)
        else:
            crisp_tr = float(treatment_capacity_fuzzy[1])
    except Exception:
        crisp_tr = float(treatment_capacity_fuzzy[1])

    return crisp_avail, crisp_dem, crisp_costs, crisp_tr

# ---------------------------------------------------------------
# Constraint evaluation (disaggregated) + penalized fitness
# ---------------------------------------------------------------

def evaluate_constraints_disaggregated(sol_dict,
                                       env_pct=0.90,
                                       crisp_avail=None,
                                       crisp_dem=None,
                                       crisp_treat=None):
    """
    Returns:
      - pure_cost (unpenalized, using 'costs' if available, otherwise Cij)
      - violations dict by constraint type
      - feasible (bool)
    Missing constraint data are gracefully skipped (no violation counted).
    """
    # Build CRISP if not provided
    if crisp_avail is None or crisp_dem is None or crisp_treat is None:
        _cr_av, _cr_dm, _cr_costs, _cr_tr = compute_crisp_inputs()
    else:
        _cr_av, _cr_dm, _cr_costs, _cr_tr = crisp_avail, crisp_dem, None, crisp_treat
    ...
    for (i, j, t) in indices:
        ci = costs.get(i, None)
        if ci is None:
            # fallback to sum of detailed cost components if available; else 0
            ci = Cij.get(i, 0.0)
        pure_cost += float(ci) * float(sol_dict[(i, j, t)])

    # Violations accumulator
    viol = {
        "R1_source_limit": 0.0,
        "R2_min_demand": 0.0,
        "R3_treatment_capacity": 0.0,
        "R_A_availability": 0.0,
        "R_D_min_sector": 0.0,
        "R_Ct_treatment_global": 0.0,
        "R_B_budget": 0.0,
        "R_Aquifer_max": 0.0,
        "R_Infra_capacity": 0.0,
        "R_Drought_min": 0.0,
        "R_Bhidro_min": 0.0,
        "R_Reserve_max": 0.0,
        "R_Global_norm": 0.0,
        # 44..50 (optional)
        "R44_micropollutants": 0.0,
        "R45_financial_efficiency": 0.0,
        "R46_infiltrations": 0.0,
        "R48_quality_monitoring": 0.0,
        "R49_infrastructure_maint": 0.0,
        "R50_education_awareness": 0.0
    }

    # Iterate periods
    for t in range(1, n_periods + 1):
        # R1: sustainable source limit (environmental percentage)
        if _cr_av:
            for i in avail_ranges.keys():
                source_sum = sum(sol_dict.get((i, j, t), 0.0) for j in demand_ranges.keys())
                limit = env_pct * _cr_av[i][t]
                if source_sum > limit:
                    viol["R1_source_limit"] += (source_sum - limit)

        # R2: minimum demand by sector
        if _cr_dm:
            for j in demand_ranges.keys():
                sector_sum = sum(sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys())
                if sector_sum < _cr_dm[j][t]:
                    viol["R2_min_demand"] += (_cr_dm[j][t] - sector_sum)

        # R3: treatment capacity CRISP for the period
        total_sum = sum(sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys() for j in demand_ranges.keys())
        if _cr_tr and total_sum > _cr_tr:
            viol["R3_treatment_capacity"] += (total_sum - _cr_tr)

        # Additional availability cap per source A[i]
        if A:
            for i in avail_ranges.keys():
                ssum = sum(sol_dict.get((i, j, t), 0.0) for j in demand_ranges.keys())
                cap = A.get(i, float("inf"))
                if ssum > cap:
                    viol["R_A_availability"] += (ssum - cap)

        # Additional minimum sectoral demand D[j]
        if D:
            for j in demand_ranges.keys():
                ssum = sum(sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys())
                dmin = D.get(j, 0.0)
                if ssum < dmin:
                    viol["R_D_min_sector"] += (dmin - ssum)

        # Global treatment capacity Ct
        if Ct > 0.0 and total_sum > Ct:
            viol["R_Ct_treatment_global"] += (total_sum - Ct)

        # Budget constraint B using detailed costs if available
        if np.isfinite(B) and (Cij or COij or CEij or CMij or CENij):
            cost_sum = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    unit = Cij.get(i, 0.0) + COij.get(i, 0.0) + CEij.get(i, 0.0) + CMij.get(i, 0.0) + CENij.get(i, 0.0)
                    cost_sum += unit * sol_dict.get((i, j, t), 0.0)
            if cost_sum > B:
                viol["R_B_budget"] += (cost_sum - B)

        # Aquifer protection
        if Aij and np.isfinite(Amax):
            aq_sum = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    aq_sum += Aij.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
            if aq_sum > Amax:
                viol["R_Aquifer_max"] += (aq_sum - Amax)

        # Infrastructure/energy capacity proxy
        if Tr and Cinfra > 0.0 and total_sum > 0.0:
            infra_sum = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    infra_sum += Tr.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
            # simple proportional cap
            if infra_sum > Cinfra * total_sum:
                viol["R_Infra_capacity"] += (infra_sum - Cinfra * total_sum)

        # Drought and hydrological minimum
        if R_drought > 0.0 and total_sum < R_drought:
            viol["R_Drought_min"] += (R_drought - total_sum)
        if B_hidro > 0.0 and total_sum < B_hidro:
            viol["R_Bhidro_min"] += (B_hidro - total_sum)

        # Strategic reserve per sector
        if np.isfinite(R_strategic):
            for j in demand_ranges.keys():
                rsum = sum(sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys())
                if rsum > R_strategic:
                    viol["R_Reserve_max"] += (rsum - R_strategic)

        # 44 Micropollutants: weighted concentration vs M_max[j]
        if M and M_max:
            for j in demand_ranges.keys():
                sector_sum = sum(sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys())
                if sector_sum > 0.0:
                    avg_c = sum(M.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0) for i in avail_ranges.keys()) / sector_sum
                    if avg_c > M_max.get(j, float("inf")):
                        viol["R44_micropollutants"] += (avg_c - M_max.get(j, float("inf"))) * sector_sum

        # 45 Financial efficiency: sum(F * x_ijt) ≤ F_max
        if F and np.isfinite(F_max):
            fsum = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    fsum += F.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
            if fsum > F_max:
                viol["R45_financial_efficiency"] += (fsum - F_max)

        # 46 Infiltrations
        if INF and np.isfinite(INF_max):
            istd = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    istd += INF.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
            if istd > INF_max:
                viol["R46_infiltrations"] += (istd - INF_max)

        # 48 Quality monitoring
        if Q_mon and Q_mon_min > 0.0:
            qsum = 0.0
            for i in avail_ranges.keys():
                for j in demand_ranges.keys():
                    qsum += Q_mon.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
            if qsum < Q_mon_min:
                viol["R48_quality_monitoring"] += (Q_mon_min - qsum)

        # 49 Infrastructure maintenance
        if Minfij is not None and Minf_min > 0.0:
            msum = 0.0
            for (i, j, _) in indices:
                try:
                    msum += float(Minfij[i-1][j-1]) * sol_dict.get((i, j, t), 0.0)
                except Exception:
                    pass
            if msum < Minf_min:
                viol["R49_infrastructure_maint"] += (Minf_min - msum)

        # 50 Education/awareness
        if E_edu is not None and Emin > 0.0:
            esum = 0.0
            for (i, j, _) in indices:
                try:
                    esum += float(E_edu[i-1][j-1]) * sol_dict.get((i, j, t), 0.0)
                except Exception:
                    pass
            if esum < Emin:
                viol["R50_education_awareness"] += (Emin - esum)

    # Global bound per decision variable
    if np.isfinite(L_norm):
        for key in indices:
            if sol_dict.get(key, 0.0) > L_norm:
                viol["R_Global_norm"] += (sol_dict[key] - L_norm)

    feasible = all(v <= 1e-12 for v in viol.values())
    return pure_cost, viol, feasible

def penalized_fitness(pure_cost, violations, pen_factor=1e6):
    """Build penalized fitness as cost + sum(pen_factor * v^2)."""
    penalty = 0.0
    for v in violations.values():
        if v > 0.0:
            penalty += pen_factor * (v ** 2)
    return pure_cost + penalty

# ---------------------------------------------------------------
# Composite sustainability indicators (Table 6-bis)
# ---------------------------------------------------------------

def compute_sustainability_kpis(sol_dict, crisp_dem, crisp_treat):
    """
    KPIs aligned with manuscript wording:
    - water_energy_efficiency (proxy): (total_supply) / (sum Tr * x_ijt)
    - normalized_cost_per_demand: pure_cost / total_CRISP_demand
    - reliability_under_stress: feasibility under 15% reduced treatment capacity
    """
    total_supply = sum(sol_dict.get(k, 0.0) for k in sol_dict)

    energy_proxy = 0.0
    for (i, j, t) in indices:
        energy_proxy += Tr.get((i, j), 0.0) * sol_dict.get((i, j, t), 0.0)
    water_energy_eff = (total_supply / energy_proxy) if energy_proxy > 0 else np.nan

    # Pure cost using 'costs' fallback
    pc = 0.0
    for (i, j, t) in indices:
        ci = costs.get(i, Cij.get(i, 0.0))
        pc += float(ci) * sol_dict.get((i, j, t), 0.0)

    total_demand = sum(crisp_dem[j][t] for j in crisp_dem for t in crisp_dem[j])
    normalized_c = (pc / total_demand) if total_demand > 0 else np.nan

    # Stress test (15% reduction in treatment capacity)
    crisp_av, crisp_dm, _, crisp_tr = compute_crisp_inputs()
    stressed_capacity = 0.85 * crisp_tr if crisp_tr else None
    pc_s, viol_s, feas_s = evaluate_constraints_disaggregated(
        sol_dict, env_pct=environmental_percentage,
        crisp_avail=crisp_av, crisp_dem=crisp_dm, crisp_treat=stressed_capacity
    )
    reliability = 1.0 if feas_s else 0.0

    return {
        "water_energy_efficiency": water_energy_eff,
        "normalized_cost_per_demand": normalized_c,
        "reliability_under_stress": reliability
    }

# ---------------------------------------------------------------
# Table S2: Penalty calibration ranges (stability under scaling)
# ---------------------------------------------------------------

def penalty_calibration_experiment(base_algorithm_names=None,
                                   pen_grid=(1e4, 1e5, 1e6, 1e7),
                                   seeds=(1, 2, 3)):
    """
    Explore stability of fitness rankings under different penalty magnitudes.
    Returns a DataFrame with mean/std fitness per algorithm and pen_factor.
    """
    if base_algorithm_names is None:
        base_algorithm_names = list(algorithms.keys())

    records = []
    crisp_av, crisp_dm, _, crisp_tr = compute_crisp_inputs()

    for pen_factor in pen_grid:
        for alg_name in base_algorithm_names:
            ...
            for seed in seeds:
                np.random.seed(seed)
                try:
                    sol, _ = algorithms[alg_name]()  # base implementation
                    sol_dict = vector_to_dict_ext(sol)
                    pc, viol, feas = evaluate_constraints_disaggregated(
                        sol_dict, env_pct=environmental_percentage,
                        crisp_avail=crisp_av, crisp_dem=crisp_dm, crisp_treat=crisp_tr
                    )
                    fit = penalized_fitness(pc, viol, pen_factor=pen_factor)
                    fit_vals.append(fit)
                except Exception:
            ...
            records.append({
                ...
                "algorithm": alg_name,
                "fitness_mean": np.nanmean(fit_vals),
                "fitness_std": np.nanstd(fit_vals),
                "n_runs": len(fit_vals)
            })
    return pd.DataFrame(records)

# ---------------------------------------------------------------
# Table S3: Hyperparameters and configuration details (declarative)
# ---------------------------------------------------------------

def collect_hyperparameters():
    """
    Build a declarative hyperparameter table. If a method does not expose
    parameters in the base script, it is recorded as N/A (to edit later).
    """
    pop_size = _ensure_int("population_size", _g("particles", 50))
    generations = _ensure_int("generations", 50)

    hp_repo = {
        "Genetic Algorithm (GA)": {"pop_size": pop_size, "generations": generations, "pc": 0.8, "pm": 0.1},
        "DE": {"pop_size": pop_size, "generations": generations, "F": 0.8, "CR": 0.9},
        "PSO": {"particles": pop_size, "iterations": generations, "w": 0.7, "c1": 1.5, "c2": 1.5},
        "Simulated Annealing (SA)": {"T0": 1000.0, "alpha": 0.99, "iters": generations * 10},
        "Tabu Search (TS)": {"tabu_size": 10, "iters": generations * 5, "neighborhood": 20},
        "Memetic Algorithm": {"pop_size": pop_size, "generations": generations, "local_iterations": 20},
        "Scatter Search": {"reference_set": "auto", "iters": "auto"},
        "Harmony Search": {"HMS": "auto", "HMCR": "auto", "PAR": "auto", "iterations": "auto"},
        "Firefly Algorithm": {"beta0": "auto", "gamma": "auto", "alpha": "auto", "iterations": "auto"},
        "Cuckoo Search": {"pa": "auto", "levy": "on", "iterations": "auto"},
        "GSA": {"G0": "auto", "alpha": "auto", "iterations": "auto"},
        "WOA": {"a_decay": "auto", "spiral_prob": "auto", "iterations": "auto"},
        "Bat Algorithm": {"A0": "auto", "r0": "auto", "fmin": "auto", "fmax": "auto", "iterations": "auto"},
        "ICA": {"empires": "auto", "assimilation": "auto", "revolution": "auto"},
        "TLBO": {"iterations": "auto"},
        "Cultural Algorithm": {"belief_update": "on", "iterations": "auto"},
        "BBO": {"immigration": "auto", "emigration": "auto", "iterations": "auto"},
        "ALO": {"random_walks": "on", "iterations": "auto"},
        "QPSO": {"mbest": "on", "iterations": "auto"},
        "Dragonfly Algorithm": {"w_sep": "auto", "w_align": "auto", "w_coh": "auto", "iterations": "auto"},
        "ACO": {"archive_size": pop_size, "iterations": generations},
        "ES": {"pop_size": pop_size, "generations": generations, "mutation_prob": 0.3},
        "EP": {"pop_size": pop_size, "generations": generations, "sigma_init": max(1.0, L_norm * 0.1) if np.isfinite(L_norm) else 1.0},
        "Artificial Bee Colony (ABC)": {"pop_size": pop_size, "limit": 20, "generations": generations},
        "Variable Neighborhood Search": {"neighborhoods": ["0.01L", "0.05L", "0.1L"], "iters": generations * 5}
    }

    rows = []
    for alg_name in algorithms.keys():
        params = hp_repo.get(alg_name, None)
        rows.append({"algorithm": alg_name, "hyperparameters": params if params is not None else "N/A"})
    return pd.DataFrame(rows)

# ---------------------------------------------------------------
# Table 4-bis: Feasibility rates and average constraint violations
# ---------------------------------------------------------------

def feasibility_and_violations(alg_names=None, seeds=tuple(range(1, 6))):
    """
    Multiple seeds per algorithm:
      - feasibility_rate (fraction of feasible solutions)
      - average violation magnitude per constraint type (μ over runs)
    """
    ...
    for alg_name in alg_names:
        ...
        for seed in seeds:
            ...
            try:
                ...
                if viol_acc is None:
                    viol_acc = {k: 0.0 for k in viol.keys()}
                for k, v in viol.items():
                    viol_acc[k] += float(v)
                feas_count += (1 if feas else 0)
                n += 1
            except Exception:
                n += 1
                continue
        if n == 0:
            continue
        avg_viol = {f"avg_{k}": (viol_acc[k] / n if viol_acc is not None else np.nan) for k in (viol_acc or {}).keys()}
        rows.append({
            "algorithm": alg_name,
            "feasibility_rate": feas_count / n,
            **avg_viol
        })
    return pd.DataFrame(rows)

# ---------------------------------------------------------------
# Table S4: Top-3 stability and pure-cost deviation across runs
# ---------------------------------------------------------------

def top3_stability_and_cost_deviation(alg_names=None, seeds=tuple(range(1, 11)), pen_factor=1e6):
    """
    Repeat runs and build rankings by penalized fitness.
    Report frequency in Top-3 and pure cost mean/std.
    """
    if alg_names is None:
        alg_names = list(algorithms.keys())

    crisp_av, crisp_dm, _, crisp_tr = compute_crisp_inputs()
    top_counts = {a: 0 for a in alg_names}
    cost_list = {a: [] for a in alg_names}
    ...
    for seed in seeds:
        ...
        for alg_name in alg_names:
            ...
            try:
                ...
                fit = penalized_fitness(pc, viol, pen_factor=pen_factor)
                fitnesses[alg_name] = fit
                pure_costs[alg_name] = pc
            except Exception:
                fitnesses[alg_name] = np.inf
                pure_costs[alg_name] = np.nan

        ordered = sorted(fitnesses.items(), key=lambda kv: kv[1])
        top3 = [name for name, _ in ordered[:3]]
        for name in top3:
            if name in top_counts:
                top_counts[name] += 1

        for name, c in pure_costs.items():
            if name in cost_list and not np.isnan(c):
                cost_list[name].append(c)

    rows = []
    total_runs = len(seeds)
    for name in alg_names:
        costs = cost_list.get(name, [])
        rows.append({
            "algorithm": name,
            "top3_frequency": top_counts.get(name, 0) / total_runs if total_runs > 0 else np.nan,
            "pure_cost_mean": float(np.mean(costs)) if len(costs) else np.nan,
            "pure_cost_std": float(np.std(costs)) if len(costs) else np.nan,
            "n_runs": total_runs
        })
    return pd.DataFrame(rows)

# ---------------------------------------------------------------
# Table 1-bis: Synthetic ranges and CRISP scenario
# ---------------------------------------------------------------

def table_1bis_synthetic_and_crisp():
    crisp_av, crisp_dm, crisp_costs, crisp_tr = compute_crisp_inputs()
    rows = []
    for i, p in avail_ranges.items():
        rows.append({
            "variable": f"availability_source_{i}",
            "min": p.get("min"), "nominal": p.get("nominal"), "max": p.get("max"),
            "crisp_period_1": crisp_av[i][1], "crisp_period_2": crisp_av[i].get(2, np.nan),
            "units": "m3"
        })
    for j, p in demand_ranges.items():
        rows.append({
            "variable": f"demand_sector_{j}",
            "min": p.get("min"), "nominal": p.get("nominal"), "max": p.get("max"),
            "crisp_period_1": crisp_dm[j][1], "crisp_period_2": crisp_dm[j].get(2, np.nan),
            "units": "m3"
        })
    for i, tpl in cost_params.items():
        cmin, cnom, cmax = tpl
        rows.append({
            "variable": f"cost_source_{i}",
            "min": cmin, "nominal": cnom, "max": cmax,
            "crisp": crisp_costs[i],
            "units": "currency_per_m3"
        })
    rows.append({
        "variable": "treatment_capacity",
        "min": treatment_capacity_fuzzy[0],
        "nominal": treatment_capacity_fuzzy[1],
        "max": treatment_capacity_fuzzy[2],
        "crisp": crisp_tr,
        "units": "m3"
    })
    return pd.DataFrame(rows)
...
# ---------------------------------------------------------------
# Master generator: write CSV + consolidated Excel workbook
# ---------------------------------------------------------------

def generate_all_required_tables(
    seeds_basic = tuple(range(1, 6)),
    seeds_top3  = tuple(range(1, 11)),
    pen_grid    = (1e4, 1e5, 1e6, 1e7),
    out_prefix  = "results_"
):
    """
    Generates:
      - Table 1-bis: Synthetic ranges and CRISP scenario
      - Table S2: Penalty calibration ranges
      - Table S3: Hyperparameters and configuration details
      - Table 4-bis: Feasibility rates and average constraint violations
      - Table S4: Top-3 stability and pure-cost deviation
      - Table 6-bis: Composite sustainability indicators
      - Table S5: Index-to (source, sector, period) mapping
    Saves as CSV and also a consolidated Excel file.
    """
    # 1-bis
    t1bis = table_1bis_synthetic_and_crisp()
    t1bis.to_csv(f"{out_prefix}table_1bis.csv", index=False)

    # S2
    s2 = penalty_calibration_experiment(
        base_algorithm_names=list(algorithms.keys()),
        pen_grid=pen_grid,
        seeds=seeds_basic
    )
    s2.to_csv(f"{out_prefix}table_S2_penalty_calibration.csv", index=False)

    # S3
    s3 = collect_hyperparameters()
    s3.to_csv(f"{out_prefix}table_S3_hyperparameters.csv", index=False)

    # 4-bis
    t4bis = feasibility_and_violations(
        alg_names=list(algorithms.keys()),
        seeds=seeds_basic
    )
    t4bis.to_csv(f"{out_prefix}table_4bis_feasibility_violations.csv", index=False)

    # S4
    s4 = top3_stability_and_cost_deviation(
        alg_names=list(algorithms.keys()),
        seeds=seeds_top3,
        pen_factor=1e6
    )
    s4.to_csv(f"{out_prefix}table_S4_top3_stability.csv", index=False)
    ...
    for alg_name in algorithms.keys():
        try:
            sol, _ = algorithms[alg_name]()
            sol_dict = vector_to_dict_ext(sol)
            kpis = compute_sustainability_kpis(sol_dict, crisp_dm, crisp_tr)
            rows_kpi.append({"algorithm": alg_name, **kpis})
        except Exception:
            rows_kpi.append({"algorithm": alg_name,
                             "water_energy_efficiency": np.nan,
                             "normalized_cost_per_demand": np.nan,
                             "reliability_under_stress": np.nan})
    s6bis = pd.DataFrame(rows_kpi)
    s6bis.to_csv(f"{out_prefix}table_6bis_sustainability_kpis.csv", index=False)

    # S5
    s5 = build_index_mapping_table()
    s5.to_csv(f"{out_prefix}table_S5_index_mapping.csv", index=False)

    # Consolidated Excel workbook
    try:
        with pd.ExcelWriter(f"{out_prefix}results_all_tables.xlsx", engine="xlsxwriter") as writer:
            t1bis.to_excel(writer, sheet_name="Table_1bis", index=False)
            s2.to_excel(writer, sheet_name="Table_S2", index=False)
            s3.to_excel(writer, sheet_name="Table_S3", index=False)
            t4bis.to_excel(writer, sheet_name="Table_4bis", index=False)
            s4.to_excel(writer, sheet_name="Table_S4", index=False)
            s6bis.to_excel(writer, sheet_name="Table_6bis", index=False)
            s5.to_excel(writer, sheet_name="Table_S5", index=False)
        _log("Consolidated Excel written: results_all_tables.xlsx")
    except Exception as e:
        _log(f"Excel export skipped ({e}); CSV files are available.")

    # Console summaries
    print("\n=== Table 1-bis (synthetic ranges and CRISP) ===")
    print(t1bis.head(10))
    print("\n=== Table S2 (penalty calibration ranges) ===")
    if not s2.empty:
        print(s2.groupby(["pen_factor"]).agg({"fitness_mean": "mean", "fitness_std": "mean"}))
    print("\n=== Table S3 (hyperparameters and configuration details) ===")
    print(s3.head(10))
    print("\n=== Table 4-bis (feasibility rates and average violations) ===")
    print(t4bis[["algorithm", "feasibility_rate"]].head(10))
    print("\n=== Table S4 (Top-3 frequency and pure cost stats) ===")
    print(s4.sort_values("top3_frequency", ascending=False).head(10))
    print("\n=== Table 6-bis (composite sustainability indicators) ===")
    print(s6bis.head(10))
    print("\n=== Table S5 (index mapping) ===")
    print(s5.head(10))

# ---------------------------------------------------------------
# Optional direct invocation when running this file as a script
# ---------------------------------------------------------------

if __name__ == "__main__":
    # For fast preprints, small seeds; for the final manuscript, extend to 30 runs and N_eval=50,000 in your methods.
    generate_all_required_tables(
        seeds_basic=tuple(range(1, 6)),      # use 1..30 for the camera-ready version
        seeds_top3=tuple(range(1, 11)),      # use 1..30 for the final stability table
        pen_grid=(1e4, 1e5, 1e6, 1e7),
        out_prefix="results_"
    )
